...
namespace AmplifyShaderEditor
{
	...
	public class MasterNodeDataCollector
	{
		...
		public void AddToDotsProperties( WirePortDataType dataType, int nodeId, string value, int orderIndex, PrecisionType precision )
		{
			...
			// string prop = "";
			string define = string.Format( IOUtils.DotsInstancedDefinesData0, UIUtils.PrecisionWirePortToCgType( precision, dataType ), value );
			string define2 = string.Format( IOUtils.DotsInstancedDefinesData1, value );
			...
			if( !m_dotsPropertiesDict.ContainsKey( value ) )
			{
				...
				var dataColl = new PropertyDataCollector( nodeId, define, orderIndex );
				m_dotsDefinesList.Add( dataColl );
				dataColl = new PropertyDataCollector( nodeId, define2, orderIndex );
				m_dotsDefinesList.Add( dataColl );
			}
		}
		...
		public void AddToUniforms( int nodeId, string dataType, string dataName, bool checkSRPBatch = false, bool excludeUniform = false, bool hybridInstanced = false)
		{
			...
			if (hybridInstanced)
			{
				value = "#ifndef UNITY_HYBRID_V1_INSTANCING_ENABLED\n" + value + "\n#endif";
			}
